...
/* Acess flags are 0x8E meaning that the entry is present and running in ring 0 and has the lower 5 bits set to the required 14, which is represented by 'E' in hex */
void isrs_install() {
...
/* Exception handling Interrupt Service Routines will point to this functions
 * All ISRs are disabled while they are being serviced to prevent an IRQ from happening and messing up kernel data structure */
void fault_handler(struct regs *r) {
	if (r->int_no < 32) {
		/* Display Exception description and go into infinite loop */
